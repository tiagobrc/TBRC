...
if [[ "${RUN_IGBLAST,,}" == "true" ]]; then
    if IGBLAST_BIN="${IGBLAST_BIN:-igblastn}" "${IGBLAST_SCRIPT}" "${IMGT_READY_FASTA_PATH}" "${IGBLAST_DIR}" "${SAMPLE}" "${IGBLAST_SPECIES:-human}" "${IGBLAST_PANEL:-ig_all}" "${IGBLAST_DB_V:-}" "${IGBLAST_DB_D:-}" "${IGBLAST_DB_J:-}" "${IGBLAST_AUX:-}" "${IGBLAST_ORGANISM:-human}" 2>/tmp/tbrc_igblast.stderr; then
...
(
    cd "${DESTINY}"
    ARCHIVE_CONTENTS=("$(basename "${IMGT_READY_FASTA_PATH}")" "$(basename "${RESULT_FASTA_PATH}")" "$(basename "${QC_DIR}")")
    if [[ -d "${IGBLAST_DIR}" ]]; then
        ARCHIVE_CONTENTS+=("$(basename "${IGBLAST_DIR}")")
    fi
    if [[ -d "${CLONALITY_DIR}" ]]; then
        ARCHIVE_CONTENTS+=("$(basename "${CLONALITY_DIR}")")
    fi
    if [[ "${ARCHIVE_FORMAT}" == "tar.gz" ]]; then
        tar -czf "${ARCHIVE_NAME}" "${ARCHIVE_CONTENTS[@]}"
    else
        zip -r "${ARCHIVE_NAME}" "${ARCHIVE_CONTENTS[@]}"
    fi
)
